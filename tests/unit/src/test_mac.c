 ...
 * \brief   What the MAC covers, and what it refuses to pass.
 *
...
ZTEST(ubi_unit, test_mac_reports_tampering_of_every_byte)
{
	 ...
	 * the CRC each round so that only the MAC can object. Magic is checked
	 * before the MAC, so those four bytes report a foreign block instead.
	 */
...
/* Tests: what the MAC binds ----------------------------------------------- */

ZTEST(ubi_unit, test_mac_binds_the_block_number)
{
...
ZTEST(ubi_unit, test_header_and_volume_table_keys_are_separate)
{
	...
	zassert_ok(ubi_impl_header_ec_serialize(
		&header, key_volume_table, TEST_PNUM, buffer, sizeof(buffer)));
...
ZTEST(ubi_unit, test_vid_mac_does_not_pass_as_ec)
{
...
ZTEST(ubi_unit, test_short_buffer_is_refused)
{
	...
	zassert_equal(-EINVAL, ubi_impl_header_ec_serialize(
				       &header, key_header, TEST_PNUM, buffer,
				       UBI_HEADER_SIZE - 1));
	zassert_equal(-EINVAL,
		      ubi_impl_header_vid_serialize(NULL, key_header, TEST_PNUM,
						    buffer, sizeof(buffer)));
	zassert_equal(UBI_HEADER_ERROR,
		      ubi_impl_header_ec_parse(buffer, UBI_HEADER_SIZE - 1,
					       key_header, TEST_PNUM,
					       TEST_ERASE_VALUE, NULL));
	zassert_equal(UBI_HEADER_ERROR,
		      ubi_impl_header_vid_parse(NULL, sizeof(buffer),
						key_header, TEST_PNUM,
						TEST_ERASE_VALUE, NULL));
}
...
ZTEST(ubi_unit, test_absent_key_is_refused)
{
	...
	zassert_equal(-EINVAL, ubi_impl_header_ec_serialize(
				       &header, PSA_KEY_ID_NULL, TEST_PNUM,
				       buffer, sizeof(buffer)));
	zassert_equal(UBI_HEADER_ERROR,
