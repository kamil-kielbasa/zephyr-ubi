...
/**
 * \brief Highest erase count on the device.
 */
static uint32_t relocate_worst_wear(const struct ubi_device *ubi);

/**
 * \brief Report whether a block's wear is far enough below the worst to be
 *        worth moving off.
 */
...
static int reclaim_choose(const struct ubi_device *ubi, uint32_t *pnum)
{
	...
	for (uint32_t candidate = 0; candidate < ubi->geometry.peb_count;
		...
		const enum ubi_peb_state state =
			ubi_peb_state_get(ubi, candidate);
...
static int reclaim_step(struct ubi_device *ubi)
{
	...
	if (0 != ret) {
		ubi_peb_retire(ubi, pnum, UBI_VOL_ID_INVALID, 0);
		return ret;
	}
...
static uint32_t relocate_worst_wear(const struct ubi_device *ubi)
{
	...
	for (uint32_t pnum = 0; pnum < ubi->geometry.peb_count; ++pnum) {
		...
		if (UBI_PEB_BAD == state || UBI_PEB_WORN_OUT == state)
			continue;

		if (ubi->blocks.erase_count[pnum] > worst)
			worst = ubi->blocks.erase_count[pnum];
	}
...
static uint32_t relocate_pending(const struct ubi_device *ubi)
{
	const uint32_t worst = relocate_worst_wear(ubi);
	uint32_t pending = 0;
...
static int relocate_choose(const struct ubi_device *ubi, uint32_t *pnum)
{
	...
	for (uint32_t candidate = 0; candidate < ubi->geometry.peb_count;
	     ++candidate) {
		if (!relocate_worthwhile(ubi, candidate, worst))
			continue;
				...
				uint32_t from, uint32_t *length)
{
	...
	while (0 != end) {
		...
		const int ret = ubi_io_read_data(ubi, pnum, at, chunk, size);
			      ...
			      uint32_t to, uint32_t length)
{
	...
	for (uint32_t at = 0; at < length; at += sizeof(chunk)) {
		const uint32_t size = MIN(sizeof(chunk), length - at);
		int ret = ubi_io_read_data(ubi, from, at, chunk, size);

		if (0 != ret)
			return ret;

		ret = ubi_io_write_data(ubi, to, at, chunk, size);
...
static int relocate_step(struct ubi_device *ubi)
{
	...
	ret = ubi_headers_read(ubi, source, &headers);

	if (0 != ret)
		return ret;

	/* The tag is checked here whatever CONFIG_UBI_VERIFY_ON_READ says:
	...
	if (UBI_HEADER_OK != headers.vid_status) {
		ubi_peb_retire(ubi, source, UBI_VOL_ID_INVALID, 0);
		return -EBADMSG;
	}
	...
	ret = relocate_data_length(ubi, source,
				   sealed ? headers.vid.data_size :
	...
	if (sealed && length == headers.vid.data_size &&
		...
		LOG_ERR("PEB %u: holds volume %u block %u and its data no "
			...
			source, vol_id, lnum);
		ubi_peb_retire(ubi, source, vol_id, lnum);
		return -EBADMSG;
	}

	uint32_t target = 0;

	ret = ubi_peb_allocate_worn(ubi, &target);
	...
	ret = ubi_vid_header_write(ubi, target, &vid);

	if (0 != ret) {
		ubi_peb_retire(ubi, target, vol_id, lnum);
		return ret;
	}
	...
	ret = ubi_volume_leb_set(ubi, vol_id, lnum, (uint16_t)target);

	if (0 != ret) {
		ubi_peb_state_set(ubi, target, UBI_PEB_UNKNOWN);
		return ret;
	}

	ubi->global_sqnum = vid.sqnum;
	ubi_peb_state_set(ubi, target, UBI_PEB_MAPPED);
	ubi_peb_state_set(ubi, source, UBI_PEB_RECLAIM);
...
static uint32_t repair_pending(const struct ubi_device *ubi)
{
	...
	for (uint32_t pnum = 0; pnum < ubi->geometry.peb_count; ++pnum) {
		if (UBI_PEB_BAD == ubi_peb_state_get(ubi, pnum))
			pending += 1;
	}
...
static int repair_choose(const struct ubi_device *ubi, uint32_t *pnum)
{
	for (uint32_t candidate = 0; candidate < ubi->geometry.peb_count;
	     ++candidate) {
		if (UBI_PEB_BAD != ubi_peb_state_get(ubi, candidate))
			continue;
...
static int repair_step(struct ubi_device *ubi)
{
	if (ubi->volume_table.degraded)
		return ubi_volumes_rewrite(ubi);
