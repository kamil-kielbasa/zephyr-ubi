...
The MAC occupies space Linux reserves as padding, and the CRC is computed the
same way — over everything before it, so over the MAC too. **An erase counter
...
`padding3` to free sixteen contiguous bytes for the MAC.
...
The MAC authenticates the whole header except itself and the CRC, with the
...
Computing the CRC last means it covers the MAC. That is what separates an
interrupted write from a forgery: a torn header fails the CRC, while a changed
field with a recomputed CRC fails the MAC.
...
Then a 16-byte AES-CMAC over everything before it.
...
comparison and a forged one fails the MAC.
